...
{
	public class CurveCorrection2
	{
		//events
		public ImageProcessing.ProgressHnd ProgressChanged;

		#region constructor
		public CurveCorrection2()
		{
		}
		#endregion
			...
			ImageProcessing.IpSettings.ItPage itPage)
		{
			...
			catch (Exception ex)
			{
				...
				throw new Exception("Big Images, CurveCorrection2, Execute(): " + ex.Message);
			}
			finally
			{
				if (itEncoder != null)
					itEncoder.Dispose();
#if DEBUG
				Console.WriteLine(string.Format("CurveCorrection2: {0}", DateTime.Now.Subtract(start).ToString()));
#endif
			}
		}
		#endregion
									...
									for (y = yBottomClipCoords; y >= 0; y--)
									{
										//if (sourceY < stripHeight)
										{
											if (sourceY >= 1)
											{
												...
												pR[1] = (byte)((yRest * pS[1]) + ((1 - yRest) * pS[1 - sStride]));
												pR[2] = (byte)((yRest * pS[2]) + ((1 - yRest) * pS[2 - sStride]));
												//if (stripY == 0) { pR[0] = 255; pR[1] = 0; pR[2] = 0; }
												//else if (stripY == 2048) { pR[0] = 0; pR[1] = 255; pR[2] = 0; }
												//else { pR[0] = 0; pR[1] = 0; pR[2] = 255; }
											}
											else if (sourceY >= 0)
											{
												pS = (pSource + ((int)(sourceY) * sStride)) + (x * pixelBytes);
