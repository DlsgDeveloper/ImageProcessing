...
namespace ImageProcessing.BigImages
{
	public class Contrast
	{
		...
		public void ChangeContrast(ImageProcessing.BigImages.ItDecoder itDecoder, string destPath, ImageProcessing.FileFormat.IImageFormat imageFormat, double contrastDelta)
		{
			...
			try
			{
				...
				for (int sourceTopLine = 0; sourceTopLine < itDecoder.Height; sourceTopLine += stripHeightMax)
				{
					...
					{
						if (contrastDelta != 0)
							ImageProcessing.Contrast.Go(strip, contrastDelta);
					...
					ProgressChanged?.Invoke((sourceTopLine + stripHeight) / (float)itDecoder.Height);
				}
			}
			catch (Exception ex)
			{
