...
namespace ImageProcessingApp
{
	...
	class _MainClass
	{
		...
		private static void Merge()
		{
			try
			{
				string[] sources = Directory.GetFiles(@"C:\Users\jirka.stybnar\TestRun\Merging\1bpp");
				List<Bitmap> images = new List<Bitmap>();

				foreach (string source in sources)
					images.Add(new Bitmap(source));

				DateTime start = DateTime.Now;

				Bitmap result = ImageProcessing.Merging.MergeVertically(images);

				Console.WriteLine("Merging: " + DateTime.Now.Subtract(start).ToString());

				result.Save(Debug.SaveToDir + "result.png", ImageFormat.Png);
				Console.ReadLine();
			}
			catch (Exception ex)
			{
		...
		private static void MergeHorizontally()
		{
			try
			{
				string source1 = @"C:\delete\it\09.jpg";
				string source2 = @"C:\delete\it\11.jpg";
				string resultFile = @"C:\delete\it\result.png";
				
				DateTime start = DateTime.Now;		
				ImageProcessing.BigImages.Merging.MergeHorizontally(source1, source2, resultFile, new ImageProcessing.FileFormat.Png(), 500);		
				Console.WriteLine("Merging: " + DateTime.Now.Subtract(start).ToString());

				/*Bitmap bitmap1 = new Bitmap(source1);
				Bitmap bitmap2 = new Bitmap(source2);
				DateTime start = DateTime.Now;
				Bitmap result = ImageProcessing.Merging.MergeHorizontally(bitmap1, bitmap2);		
				result.Save(resultFile, ImageFormat.Png);
				Console.WriteLine("Merging: " + DateTime.Now.Subtract(start).ToString());*/
